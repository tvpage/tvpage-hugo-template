!function(a, b) {
    ...
    function c() {
        !function(a, b, c, j) {
            Utils.render(c, b.body), Utils.loadData(j, c, function(b) {
                if (b && b.length) {
                    a.parent && a.parent.postMessage({
                        event: ("tvp_" + j.id).replace(/-/g, "_") + ":handle_init",
                        data: b
                    }, "*");
                    var k = JSON.parse(JSON.stringify(j));
                    if (k.data = b || [], playlistOpt = !!Utils.isset(j, "playlist") && j.playlist, 
                    playlistOpt) {
                        k.onFullscreenChange = function() {
                            f.hideMenu();
                        }, e = new Player("tvp-player-el-" + c, k), d = e.el;
                        var l = JSON.parse(JSON.stringify(j));
                        l.data = b || [], l.channelVideosPage = 0, f = new Menu(e, l), Menu.prototype.loadMore = function() {
                            h || i || (l.channelVideosPage++, i = !0, Utils.loadData(l, c, function(a) {
                                if (a.length > g) return void f.deleteDivs();
                                i = !1, h = !a.length || a.length < g, e.addData(a), f.render(a);
                            }));
                        };
                        var m = setInterval(function() {
                            n(e, j);
                        }, 500), n = function(a, b) {
                            a.isReady && b && (clearInterval(m), f.init());
                        };
                    } else e = new Player("tvp-player-el-" + c, k);
                }
            });
        }(a, b, Utils.random(), Utils.getSettings());
    }
    var d, e, f, g = ((b.body.getAttribute("data-id") || "").replace(/-/g, "_"), 6), h = !1, i = !1, j = function(a) {
        return void 0 === a;
    };
    if (j(a.Utils) || j(a.Player)) {
        var k = 0;
        !function b() {
            setTimeout(function() {
                (j(a.Utils) || j(a.Player)) && ++k < 50 ? b() : c();
            }, 150);
        }();
    } else c();
}(window, document);