!function(a, b) {
        ...
        if (c && d(i) && d(i.data) && !(i.data.length <= 0)) {
            ...
            this.getOption = function(a) {
                ...
                if (a) {
                    ...
                    e ? this.instance.cueVideo(a) : this.instance.loadVideo(a);
                }
    ...
    function c(c, d) {
        ...
        this.init = function() {
            e.render(), e.setDOMElements(), e.bindMenuEvent(), e.bindClickEvent(), e.bindLoadMoreEvent(), 
            e.hideMenuEvents(), e.listenToResize();
        }, this.setDOMElements = function() {
            e.scrollMenu = b.querySelectorAll(".ss-content")[0], e.tvpVid = b.querySelectorAll(".tvp-video"), 
            e.hamburguer = b.getElementById("tvp-hamburger-container"), e.toggles = b.querySelectorAll(".tvp-hamburger"), 
            e.playerCont = b.querySelectorAll(".tvp-player")[0], e.noVideosContainer = b.getElementById("tvp-no-videos-container"), 
            e.tvpNoVideos = b.getElementsByClassName("tvp-no-videos"), e.scrollBar = b.getElementsByClassName("ss-scroll")[0], 
            e.slideMenu = b.getElementById("tvp-slide-menu");
        }, this.render = function() {
            var a = d.data || [];
            if (!(a.length < 1)) {
                e.fullScreenMenu(), e.hiddenMenu = b.getElementById("tvp-hidden-menu");
                var c = b.getElementsByClassName("tvp-video-details"), f = b.createElement("div"), g = b.createElement("div");
                f.setAttribute("id", "tvp-clearfix"), g.setAttribute("id", "tvp-no-videos-container"), 
                e.hiddenMenu.appendChild(g), e.hiddenMenu.insertBefore(f, g), e.vidCount = 0;
                for (var h = 0; h < a.length; h++) {
                    e.vidCount++;
                    var i = a[h];
                    if (e.allVideos.push(i), i.title = Utils.trimText(i.title, 100), i.duration = Utils.formatDuration(i.duration), 
                    f.innerHTML += Utils.tmpl(d.templates["menu-item"], i), Utils.isset(d, "menu_item_play_category_tag_attribute") && ("" + d.menu_item_play_category_tag_attribute).trim().length) {
                        var j = i[d.menu_item_play_category_tag_attribute];
                        if (j) {
                            j = j.replace(/_/g, " ");
                            var k = b.createDocumentFragment(), l = b.createElement("div");
                            l.classList.add("tvp-category-tag"), l.innerHTML += j, k.appendChild(l), c[h].appendChild(k);
                        }
                    }
                    if ("static" !== e.dataMethod && a.length > 4) {
                        var m = b.createDocumentFragment(), n = b.createElement("div");
                        n.classList.add("tvp-no-videos"), m.appendChild(n), g.appendChild(m);
                    }
                }
                ...
                e.hiddenMenu.style.cssText = "height:" + (b.querySelectorAll(".tvp-player")[0].offsetHeight - 36) + "px;", 
            ...
            function b() {
                var a = e.playerCont.offsetHeight - 36 + "px;";
            ...
            e.setDOMElements(), e.bindClickEvent();
